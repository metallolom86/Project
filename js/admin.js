class Admin extends HTMLElement {
  ...
  addElem() {
    ...
    const url = "https://fea13-anton.glitch.me/";
    ...
    delItem.onclick = async function(event) {
      inputUsers.value = "";
      reset();
      let info = await (await fetch(`${url}items`)).json();
    ...
    function dele() {
      fetch(`${url}${catDel.value}/${number.value}`, {
        method: "DELETE"
      });
      resetDel();
    }
    ...
    function putCardItem() {
      fetch(`${url}items/${penId.textContent}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          title: penTitle.value,
          category: penCategory.value,
          text: penText.value,
          price: penPrice.value,
          photo: penImg.src
        })
      });
      resetInput();
    ...
    searchOrder.onclick = async function(event) {
      ...
      orderUser.value
        ? (user = info.filter(x => x.user === orderUser.value.toLowerCase()))
        : null;
      orderDate.value
        ? (user = info.filter(x => x.date === orderDate.value))
        : null;
      document.querySelector("main").innerHTML = "";
      let full = document.createElement("div");
      full.classList.add("full");
    ...
    cangOrder.onclick = function(event) {
      fetch(`${url}orders/${numbOrder.value}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json"
        },
    ...
    butOrderDel.onclick = function(event) {
      fetch(`${url}orders/${numOrderDel.value}`, {
        method: "DELETE"
      });
